import {Button, Exponent, Panel} from "./_snowpack/pkg/@repcomm/exponent-ts.js";
export const CAROUSEL3D_STYLES = new Exponent().make("style").setTextContent(`
.carousel3d {
...
export class Carousel3d extends Panel {
  constructor() {
    ...
    this.untransformer = new Panel().addClasses("carousel3d-untransformer").mount(this);
    this.measurementItem = new Carousel3dItem().addClasses("carousel3d-item-untransformed").mount(this.untransformer);
    this.items = new Set();
    this.buttonsContainer = new Panel().addClasses("carousel3d-buttons-container").mount(this);
    this.btnAdd = new Button().addClasses("carousel3d-button").setTextContent("Add").on("click", () => {
      this.create();
    }).mount(this.buttonsContainer);
    this.btnRemove = new Button().addClasses("carousel3d-button").setTextContent("Remove").on("click", () => {
      let firstItem;
    ...
    this.btnPrev = new Button().addClasses("carousel3d-button").setTextContent("<").on("click", () => {
      this.offsetIndex++;
      this.updateItems();
    }).mount(this.buttonsContainer);
    this.btnNext = new Button().addClasses("carousel3d-button").setTextContent(">").on("click", () => {
      this.offsetIndex--;
      this.updateItems();
    }).mount(this.buttonsContainer);
    this.offsetIndex = 0;
  }
  add(autoUpdate = true, ...items) {
